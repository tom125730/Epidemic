import requests, httpx, asyncio, json, time
...
async def main(event, context):
    ...
    async with httpx.AsyncClient(http2=True) as client:
        ...
        for i in range(len(user_data)):
            try:
                r = Epidemic(i, user_data[i]['_name'], user_data[i]['_username'], user_data[i]['_password'], UA_data[i % len(UA_data)], user_data[i]['_notify'], user_data[i]['_token'])
